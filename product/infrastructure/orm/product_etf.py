...
class ProductETFORM(Base):
    ...
    nPpTotAmt = Column(BigInteger, nullable=True)            # 순자산총액
    stLstgCnt = Column(BigInteger, nullable=True)            # 상장좌수
    bssldxldxNm = Column(String(255), nullable=True)   # 기초지수_지수명
    bssldxClpr = Column(Float, nullable=True)          # 기초지수_종가
    basDt = Column(DateTime, default=datetime.utcnow)  # 기준일자
    dpr = Column(Integer, nullable=True)               # 종가
